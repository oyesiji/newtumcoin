...
const char * const BITCOIN_CONF_FILENAME = "newtumcoin.conf";
const char * const BITCOIN_PID_FILENAME = "newtumcoin.pid";
...
#else
    const char* pszModule = "newtumcoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Newtumcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Newtumcoin";
#else
    // Unix
    return pathRet / ".newtumcoin";
#endif
#endif
}
